...
BEGIN
  ...
  -- decisions pointing at work / photos that no longer exist must never block a shop
  DELETE FROM public.field_review_decisions d WHERE d.shop_id=p_shop_id AND d.stage=p_stage AND NOT (
    (d.entity_type IN ('measurement','work_item') AND EXISTS (SELECT 1 FROM public.work_items w WHERE w.id=d.entity_id AND w.shop_id=p_shop_id))
    OR (d.entity_type='survey_photo' AND EXISTS (SELECT 1 FROM public.survey_photos x WHERE x.id=d.entity_id))
    OR (d.entity_type='installation_photo' AND EXISTS (SELECT 1 FROM public.installation_proofs x WHERE x.id=d.entity_id)));
  ...
  IF p_stage='survey' THEN
    ...
    SELECT count(*) INTO v_redo FROM public.field_review_decisions WHERE stage='survey' AND shop_id=p_shop_id AND decision='redo';
  ELSIF p_stage='installation' THEN
    ...
    SELECT count(*) INTO v_redo FROM public.field_review_decisions WHERE stage='installation' AND shop_id=p_shop_id AND decision='redo'
  ...
  END IF;
  -- approving everything that is left also closes stray/legacy corrections that have no live "redo" behind them
  IF p_decision='approved' THEN
    UPDATE public.field_corrections c SET status='resolved', resolved_at=now()
    WHERE c.shop_id=p_shop_id AND c.stage=p_stage AND c.status IN ('open','resubmitted')
      AND NOT EXISTS (SELECT 1 FROM public.field_review_decisions d WHERE d.stage=p_stage AND d.decision='redo'
        AND d.entity_id IN (COALESCE(c.work_item_id,'00000000-0000-0000-0000-000000000000'),COALESCE(c.survey_photo_id,'00000000-0000-0000-0000-000000000000'),COALESCE(c.installation_proof_id,'00000000-0000-0000-0000-000000000000')));
  END IF;
  ...
  IF v_pending=0 AND v_redo=0 AND v_open=0 THEN
    IF p_stage='survey' THEN
      ...
      ELSE
        ...
        UPDATE public.shop_assignments SET status='completed', completed_at=COALESCE(completed_at,now()) WHERE shop_id=p_shop_id AND role='surveyor';
    ...
    ELSIF p_stage='installation' THEN
      ...
      UPDATE public.shop_assignments SET status='completed', completed_at=COALESCE(completed_at,now()) WHERE shop_id=p_shop_id AND role='installer';
...
-- ---------- 9. remember the size EXACTLY as the surveyor entered it (inch / ft / m / cm) ----------
-- survey_* / approved_* / installed_* stay in feet: that is the single base every total is calculated from
-- (then shown in the work-order unit). entered_* only exists so people SEE the size the way it was surveyed.
ALTER TABLE public.work_items
  ADD COLUMN IF NOT EXISTS entered_width numeric,
  ADD COLUMN IF NOT EXISTS entered_height numeric,
  ADD COLUMN IF NOT EXISTS entered_width_unit text,
  ADD COLUMN IF NOT EXISTS entered_height_unit text;
UPDATE public.work_items SET entered_width=survey_width, entered_height=survey_height, entered_width_unit='ft', entered_height_unit='ft'
WHERE entered_width IS NULL AND survey_width IS NOT NULL AND survey_height IS NOT NULL;

-- ---------- 10. work item name is ALWAYS filled (work type -> PO line item -> material) ----------
CREATE OR REPLACE FUNCTION public.autofill_work_item_name() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE v text;
BEGIN
  IF NULLIF(trim(coalesce(NEW.work_type_name,'')),'') IS NULL OR lower(trim(NEW.work_type_name)) IN ('work item','other') THEN
    IF NEW.work_type_id IS NOT NULL THEN SELECT name INTO v FROM public.work_types WHERE id=NEW.work_type_id; END IF;
    IF v IS NULL AND NEW.po_line_item_id IS NOT NULL THEN
      SELECT COALESCE(wt.name, NULLIF(trim(pli.description),'')) INTO v FROM public.po_line_items pli LEFT JOIN public.work_types wt ON wt.id=pli.work_type_id WHERE pli.id=NEW.po_line_item_id;
    END IF;
    IF v IS NULL THEN v := NULLIF(trim(coalesce(NEW.material,'')),''); END IF;
    IF v IS NOT NULL THEN NEW.work_type_name := v; END IF;
  END IF;
  RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS trg_autofill_work_item_name ON public.work_items;
CREATE TRIGGER trg_autofill_work_item_name BEFORE INSERT OR UPDATE OF work_type_id, po_line_item_id, material, work_type_name ON public.work_items
FOR EACH ROW EXECUTE FUNCTION public.autofill_work_item_name();
UPDATE public.work_items SET work_type_name=work_type_name WHERE NULLIF(trim(coalesce(work_type_name,'')),'') IS NULL OR lower(trim(work_type_name)) IN ('work item','other');

-- ---------- 11. owner/admin: change a shop's stage manually from Shop Details ----------
CREATE OR REPLACE FUNCTION public.set_shop_stage(p_shop_id uuid, p_status text) RETURNS text LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE v_org uuid; v_old text;
BEGIN
  SELECT organization_id, status INTO v_org, v_old FROM public.shops WHERE id=p_shop_id;
  IF v_org IS NULL OR v_org <> public.current_org_id() THEN RAISE EXCEPTION 'Shop not accessible'; END IF;
  IF public.current_role() NOT IN ('agency_owner','admin','demo') THEN RAISE EXCEPTION 'Only Owner/Admin can change the stage'; END IF;
  IF v_old = p_status THEN RETURN v_old; END IF;
  -- "installed" is only reachable through installation_review (keeps the review trail consistent)
  IF p_status = 'installed' AND v_old <> 'installation_review' THEN
    UPDATE public.shops SET status='installation_review' WHERE id=p_shop_id;
  END IF;
  -- moving a shop back before installation review starts a fresh review: old installation decisions must not re-approve it
  IF p_status IN ('pending','assigned','survey_started','surveyed','approval_pending','approved','design_pending','designing','design_ready','in_review','design_approved',
                  'production_pending','in_production','production_ready','production_hold','production_done','dispatched','installation_pending','installing') THEN
    DELETE FROM public.field_review_decisions WHERE shop_id=p_shop_id AND stage='installation';
  END IF;
  UPDATE public.shops SET status=p_status WHERE id=p_shop_id;
  RETURN p_status;
END $$;
GRANT EXECUTE ON FUNCTION public.set_shop_stage(uuid,text) TO authenticated;


-- ---------- 12. repair: approved installations whose shop never became "installed" ----------
DO $$ BEGIN PERFORM set_config('app.review_rpc','on',true); END $$;
UPDATE public.shops s SET status='installed'
WHERE s.status IN ('installation_review','installation_pending','installing')
  AND (SELECT j.review_status FROM public.installation_jobs j WHERE j.shop_id=s.id ORDER BY j.created_at DESC LIMIT 1)='approved';
UPDATE public.shop_assignments a SET status='completed', completed_at=COALESCE(a.completed_at, now())
FROM public.shops s WHERE s.id=a.shop_id AND s.status IN ('installed','billed') AND a.role='installer' AND a.status<>'completed';
UPDATE public.field_corrections c SET status='cancelled', resolved_at=COALESCE(c.resolved_at, now())
FROM public.shops s WHERE s.id=c.shop_id AND s.status IN ('installed','billed') AND c.status IN ('open','resubmitted');
UPDATE public.work_items w SET status='installed' FROM public.shops s
WHERE s.id=w.shop_id AND s.status IN ('installed','billed') AND NOT w.excluded_from_calculations AND w.status NOT IN ('installed');

-- ---------- 13. SELF-HEALING: whenever every live work item is approved, the shop IS installed ----------
-- Works no matter which screen / old code path wrote the decisions.
CREATE OR REPLACE FUNCTION public.reconcile_installation_shop(p_shop_id uuid) RETURNS boolean LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE v_status text; v_org uuid; v_active int; v_unapproved int; v_redo int; v_job uuid; v_name text; v_inst uuid;
BEGIN
  SELECT status, organization_id, name INTO v_status, v_org, v_name FROM public.shops WHERE id=p_shop_id;
  IF v_status IS NULL OR v_status NOT IN ('installation_review','installation_pending','installing') THEN RETURN false; END IF;
  SELECT id, installer_id INTO v_job, v_inst FROM public.installation_jobs WHERE shop_id=p_shop_id ORDER BY created_at DESC LIMIT 1;
  IF v_job IS NULL THEN RETURN false; END IF;
  SELECT count(*) INTO v_active FROM public.work_items WHERE shop_id=p_shop_id AND NOT excluded_from_calculations;
  IF v_active = 0 THEN RETURN false; END IF;
  SELECT count(*) INTO v_unapproved FROM public.work_items w WHERE w.shop_id=p_shop_id AND NOT w.excluded_from_calculations
    AND NOT EXISTS (SELECT 1 FROM public.field_review_decisions d WHERE d.stage='installation' AND d.entity_type='work_item' AND d.entity_id=w.id AND d.decision='approved');
  IF v_unapproved > 0 THEN RETURN false; END IF;
  SELECT count(*) INTO v_redo FROM public.field_review_decisions d WHERE d.shop_id=p_shop_id AND d.stage='installation' AND d.decision='redo'
    AND ((d.entity_type='work_item' AND EXISTS (SELECT 1 FROM public.work_items w WHERE w.id=d.entity_id AND w.shop_id=p_shop_id AND NOT w.excluded_from_calculations))
      OR (d.entity_type='installation_photo' AND EXISTS (SELECT 1 FROM public.installation_proofs x WHERE x.id=d.entity_id)));
  IF v_redo > 0 THEN RETURN false; END IF;

  PERFORM set_config('app.review_rpc','on',true);
  UPDATE public.shops SET status='installed' WHERE id=p_shop_id;
  UPDATE public.installation_jobs SET review_status='approved', reviewed_at=COALESCE(reviewed_at, now()), status=CASE WHEN status IN ('assigned','started') THEN 'completed' ELSE status END
    WHERE shop_id=p_shop_id AND review_status IN ('pending','rejected','not_applicable');
  UPDATE public.shop_assignments SET status='completed', completed_at=COALESCE(completed_at, now()) WHERE shop_id=p_shop_id AND role='installer';
  UPDATE public.work_items SET status='installed' WHERE shop_id=p_shop_id AND NOT excluded_from_calculations AND status<>'installed';
  UPDATE public.field_corrections SET status='resolved', resolved_at=now() WHERE shop_id=p_shop_id AND stage='installation' AND status IN ('open','resubmitted');
  PERFORM public._notify(v_org, v_inst, 'Installation approved', 'Installation for '||COALESCE(v_name,'shop')||' was approved.', 'success', '/mobile');
  RETURN true;
END $$;
GRANT EXECUTE ON FUNCTION public.reconcile_installation_shop(uuid) TO authenticated;

CREATE OR REPLACE FUNCTION public.trg_reconcile_after_decision() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE v_shop uuid;
BEGIN
  v_shop := COALESCE(NEW.shop_id, OLD.shop_id);
  IF COALESCE(NEW.stage, OLD.stage)='installation' AND v_shop IS NOT NULL THEN PERFORM public.reconcile_installation_shop(v_shop); END IF;
  RETURN COALESCE(NEW, OLD);
END $$;
DROP TRIGGER IF EXISTS trg_reconcile_after_decision ON public.field_review_decisions;
CREATE TRIGGER trg_reconcile_after_decision AFTER INSERT OR UPDATE OR DELETE ON public.field_review_decisions
FOR EACH ROW EXECUTE FUNCTION public.trg_reconcile_after_decision();

-- marking the last undecided work "not available" can also complete a shop
CREATE OR REPLACE FUNCTION public.trg_reconcile_after_availability() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
BEGIN
  IF NEW.excluded_from_calculations IS DISTINCT FROM OLD.excluded_from_calculations THEN PERFORM public.reconcile_installation_shop(NEW.shop_id); END IF;
  RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS trg_reconcile_after_availability ON public.work_items;
CREATE TRIGGER trg_reconcile_after_availability AFTER UPDATE OF excluded_from_calculations ON public.work_items
FOR EACH ROW EXECUTE FUNCTION public.trg_reconcile_after_availability();

-- repair every shop that is already in this state today
DO $$ DECLARE r record; BEGIN
  FOR r IN SELECT id FROM public.shops WHERE status IN ('installation_review','installation_pending','installing') LOOP
    PERFORM public.reconcile_installation_shop(r.id);
  END LOOP;
END $$;
