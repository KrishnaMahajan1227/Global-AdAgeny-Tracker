...
import { LENGTH_UNIT_OPTIONS, toFeet, trimNum } from '@/lib/units';
...
export function SurveyCorrectionFixer({ shopId, onExit }: { shopId: string; onExit: () => void }) {
  ...
  const form = (it: any) => forms[it.id] || { w: String(it.entered_width ?? trimNum(it.survey_width)), h: String(it.entered_height ?? trimNum(it.survey_height)), unit: it.entered_width_unit || 'ft', qty: String(it.survey_quantity ?? 1), notes: it.survey_notes || '' };
  ...
  async function saveItem(it: any, corrId: string) {
    ...
    try {
      ...
      const { error } = await supabase.from('work_items').update({ survey_width: w, survey_height: h, survey_unit: 'ft', entered_width: parseFloat(f.w), entered_height: parseFloat(f.h), entered_width_unit: f.unit, entered_height_unit: f.unit, survey_quantity: q, survey_area: Math.round(w * h * q * 100) / 100, survey_notes: f.notes || null }).eq('id', it.id);
      if (error) throw error;
