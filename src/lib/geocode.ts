...
/**
 * The mirror image of geocodeAddress: given a GPS point (typically the
 * device's own current location), returns the best-guess address/city/
 * state at that point. Used by the Installer app's Direct Install flow
 * (migration 0077) so a ground-crew member who paints a wall the company
 * never gave an address for can tap "Use My Location" and get a starting
 * address/city/state to correct instead of typing all three blind.
 * Never blocks the flow — every field it fills stays editable, and a
 * failure here (no signal, quota, ZERO_RESULTS) just leaves the fields
 * blank for manual entry instead of throwing.
 */
  ...
  return new Promise((resolve, reject) => {
    geocoder.geocode({ location: { lat, lng } }, (results: any, status: string) => {
      if (status === 'OK' && results && results[0]) {
        ...
        resolve({ formattedAddress: results[0].formatted_address as string, city, state });
      } else {
        reject(new Error(`Could not resolve an address for this location (${status}).`));
      }
    });
  });
}
