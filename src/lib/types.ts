...
// The four Work Order scopes a client can hand an agency (Architecture
// v2.0 §3.4 + migration 0077):
// - survey_install: full Dealer Boards pipeline (survey -> design ->
//   production -> installation), the original/default type.
// - supply_only: production + zone-wise dispatch, no field survey or
//   on-site install job.
// - custom: an Admin/Owner ticks whichever of survey/design/production/
//   installation apply, per line item, by hand.
// - direct_install: the fixed "wall painting / on-ground campaign" preset
//   — no survey, no design, no production; the ground crew's own on-site
//   photo+size submission (InstallerPage's "Direct Install" tab) IS the
//   installation record. See migration 0077.
export type FulfillmentType = 'survey_install' | 'supply_only' | 'custom' | 'direct_install';
...
// One row per (Work Order, ground-crew member) — the PO-level equivalent
// of ShopAssignment for Direct Install Work Orders, which have no shop to
// assign against until the ground crew logs one (migration 0078).
...
// Mirrors public.v_po_work_context (migration 0029, `name` added by 0068)
// — the non-financial, PO-level counterpart to POLineItemWorkContext
// below. Every field-safe screen (Surveyor, Installer's Direct Install
// tab) that needs to show "which Work Order is this" without touching
// the rate/amount-carrying `purchase_orders` table reads this instead.
export interface POWorkContext {
  id: string;
  organization_id: string;
  client_id: string;
  project_id: string | null;
  // migration 0079 — the agency's OWN campaign-equivalent (projects
  // table). Per PurchaseOrdersPage.tsx: at most one of project_id/
  // campaign_id is ever set on a given PO — project_id for one the
  // agency created itself, campaign_id (below) for one grouped by the
  // client on a client-created PO.
  project_name: string | null;
  po_number: string;
  po_date: string;
  fulfillment_type: FulfillmentType;
  status: 'active' | 'closed' | 'cancelled';
  name: string | null;
  // migration 0079 — nullable: campaigns are client-side and optional,
  // most agency-created POs will never have one (they'll have
  // project_id/project_name instead, or neither).
  campaign_id: string | null;
