...
import { startOfWeek, endOfWeek, startOfMonth, endOfMonth, startOfYear, endOfYear, format as formatDate } from 'date-fns';
...
import { Plus, Pencil, Trash2, Users, Building2, Tag, DollarSign, FileText, Shield, Package, Link2, Ban, RotateCcw, Gauge, Search, Truck, AlertTriangle, Clock, ListChecks, TrendingUp, Copy, Check, Loader2, BarChart3, CalendarRange, XCircle, Download } from 'lucide-react';

type Tab = 'users' | 'org' | 'platform-clients' | 'rate-cards' | 'work-types' | 'consumables' | 'audit' | 'export' | 'workload' | 'performance' | 'vehicle-loads';
...
export default function OwnerConsolePage() {
  ...
  return (
    <div>
      ...
      {tab === 'performance' && <TeamPerformanceTab />}
...
// "Who did how much work, in a period you actually choose" — the
// complement to Team Workload above (which only ever shows CURRENT open/
// in-progress load and a fixed "this month" completed count). This reads
// straight from the same source-of-truth tables v_team_workload itself is
// built from (shop_assignments for surveyor/installer, design_tasks for
// designer), plus production_orders for the Production/Printing role, and
// surveys/installation_jobs for the quality signal (how much of a
// person's output got sent back) — filtered to whatever date range is
// picked, computed client-side so Weekly/Monthly/Yearly/Custom Range all
// need the exact same queries, just different date boundaries. No new
// migration: every column read here already exists and is already
// written by the app (see DesignerPage.tsx's approve mutations, which
// this pass also fixed to actually stamp design_tasks.completed_at —
// previously never written, so a designer's completed count here and on
// Team Workload would have silently stayed zero forever).
type PerfPreset = 'week' | 'month' | 'year' | 'custom';
type PerfRoleFilter = 'all' | 'surveyor' | 'designer' | 'installer' | 'printing';
type PerfSortKey = 'full_name' | 'completed' | 'sentBack';
...
interface TeamPerformanceRow {
  ...
  role: string;
  completed: number;
  /** null = this role has no "sent back for rework" concept (Production). */
  sentBack: number | null;
}

const PERF_PRESETS: { id: PerfPreset; label: string }[] = [
...
const PERF_ROLE_TABS: { id: PerfRoleFilter; label: string }[] = [
  { id: 'all', label: 'All' },
  { id: 'surveyor', label: 'Surveyors' },
  { id: 'designer', label: 'Designers' },
  { id: 'installer', label: 'Installers' },
  { id: 'printing', label: 'Production' },
];

/** Groups a list of rows by one string column into counts — the one bit
 *  of aggregation math every source query below needs, done once here
 *  instead of six near-identical reduce() calls. */
function countBy(rows: Record<string, any>[], key: string): Map<string, number> {
...
function TeamPerformanceTab() {
  ...
  const [preset, setPreset] = useState<PerfPreset>('month');
  const [customStart, setCustomStart] = useState(() => formatDate(startOfMonth(new Date()), 'yyyy-MM-dd'));
  const [customEnd, setCustomEnd] = useState(() => formatDate(new Date(), 'yyyy-MM-dd'));
  const [search, setSearch] = useState('');
  const [roleFilter, setRoleFilter] = useState<PerfRoleFilter>('all');
  const [sortKey, setSortKey] = useState<PerfSortKey>('completed');
  const [sortDir, setSortDir] = useState<'asc' | 'desc'>('desc');
  ...
  const rangeLabel = preset === 'custom'
    ? `${formatDate(range.start, 'dd MMM yyyy')} – ${formatDate(range.end, 'dd MMM yyyy')}`
    : PERF_PRESETS.find((p) => p.id === preset)?.label || '';
  const startIso = range.start.toISOString();
  const endIso = range.end.toISOString();

  const { data: people } = useQuery({
    queryKey: ['perf-people', orgId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('profiles').select('id, full_name, role')
        .eq('organization_id', orgId).eq('is_active', true)
        .in('role', ['surveyor', 'designer', 'installer', 'printing'])
        .order('full_name');
  ...
  const { data: counts, isLoading } = useQuery({
    queryKey: ['team-performance', orgId, startIso, endIso],
    queryFn: async () => {
      const [
        surveysCompletedRes, surveysSentBackRes,
        installsCompletedRes, installsSentBackRes,
        designsCompletedRes, designsRejectedRes,
        productionCompletedRes,
      ] = await Promise.all([
        // Surveyor "completed" = their own assignment closed out — the
        // same field shop_assignments.completed_at already gets stamped
        // with when a survey is actually submitted (syncManager.ts).
        supabase.from('shop_assignments').select('user_id')
          .eq('organization_id', orgId).eq('role', 'surveyor').eq('status', 'completed')
          .gte('completed_at', startIso).lte('completed_at', endIso),
        // Quality signal: how many of their submissions got sent back,
        // dated by when the REVIEW happened (not the original submission).
        supabase.from('surveys').select('surveyor_id')
          .eq('organization_id', orgId).in('status', ['rejected', 'correction_requested'])
          .gte('reviewed_at', startIso).lte('reviewed_at', endIso),
        supabase.from('shop_assignments').select('user_id')
          .eq('organization_id', orgId).eq('role', 'installer').eq('status', 'completed')
          .gte('completed_at', startIso).lte('completed_at', endIso),
        supabase.from('installation_jobs').select('installer_id')
          .eq('organization_id', orgId).eq('review_status', 'rejected')
          .gte('reviewed_at', startIso).lte('reviewed_at', endIso),
        supabase.from('design_tasks').select('designer_id')
          .eq('organization_id', orgId).eq('status', 'approved').not('designer_id', 'is', null)
          .gte('completed_at', startIso).lte('completed_at', endIso),
        supabase.from('design_tasks').select('designer_id')
          .eq('organization_id', orgId).eq('status', 'rejected').not('designer_id', 'is', null)
          .gte('updated_at', startIso).lte('updated_at', endIso),
        // Production has no "sent back" concept in this pipeline (no
        // review/approval gate on production_orders beyond completed) —
        // only a completed count.
        supabase.from('production_orders').select('assigned_to')
          .eq('organization_id', orgId).eq('status', 'completed').not('assigned_to', 'is', null)
          .gte('updated_at', startIso).lte('updated_at', endIso),
      ]);

      for (const res of [surveysCompletedRes, surveysSentBackRes, installsCompletedRes, installsSentBackRes, designsCompletedRes, designsRejectedRes, productionCompletedRes]) {
        if (res.error) throw new Error(`Could not load performance data: ${res.error.message}`);
      }

      return {
        surveysCompleted: countBy(surveysCompletedRes.data || [], 'user_id'),
        surveysSentBack: countBy(surveysSentBackRes.data || [], 'surveyor_id'),
        installsCompleted: countBy(installsCompletedRes.data || [], 'user_id'),
        installsSentBack: countBy(installsSentBackRes.data || [], 'installer_id'),
        designsCompleted: countBy(designsCompletedRes.data || [], 'designer_id'),
        designsRejected: countBy(designsRejectedRes.data || [], 'designer_id'),
        productionCompleted: countBy(productionCompletedRes.data || [], 'assigned_to'),
      };
    },
    enabled: !!orgId,
  });

  const allRows: TeamPerformanceRow[] = (people || []).map((p) => {
    if (p.role === 'surveyor') {
      return { user_id: p.id, full_name: p.full_name, role: p.role, completed: counts?.surveysCompleted.get(p.id) || 0, sentBack: counts?.surveysSentBack.get(p.id) || 0 };
    }
    if (p.role === 'installer') {
      return { user_id: p.id, full_name: p.full_name, role: p.role, completed: counts?.installsCompleted.get(p.id) || 0, sentBack: counts?.installsSentBack.get(p.id) || 0 };
    }
    if (p.role === 'designer') {
      return { user_id: p.id, full_name: p.full_name, role: p.role, completed: counts?.designsCompleted.get(p.id) || 0, sentBack: counts?.designsRejected.get(p.id) || 0 };
    }
    return { user_id: p.id, full_name: p.full_name, role: p.role, completed: counts?.productionCompleted.get(p.id) || 0, sentBack: null };
  });

  const summary = {
    people: allRows.length,
    totalCompleted: allRows.reduce((sum, r) => sum + r.completed, 0),
    totalSentBack: allRows.reduce((sum, r) => sum + (r.sentBack || 0), 0),
    topPerformer: allRows.slice().sort((a, b) => b.completed - a.completed)[0] || null,
  };
  ...
  const filteredRows = allRows
    ...
    .sort((a, b) => {
      ...
      const av = sortKey === 'completed' ? a.completed : (a.sentBack ?? 0);
      const bv = sortKey === 'completed' ? b.completed : (b.sentBack ?? 0);
  ...
  function exportToExcel() {
    const data = filteredRows.map((r) => ({
      ...
      'Sent Back / Rejected': r.sentBack ?? '—',
      'Period': rangeLabel,
    }));
    const ws = XLSX.utils.json_to_sheet(data);
    ws['!cols'] = [{ wch: 22 }, { wch: 18 }, { wch: 12 }, { wch: 18 }, { wch: 22 }];
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Team Performance');
    const safeLabel = rangeLabel.replace(/[^a-z0-9]+/gi, '-').replace(/^-+|-+$/g, '');
    XLSX.writeFile(wb, `team-performance-${safeLabel || 'report'}.xlsx`);
  }

  return (
    <div>
      <p className="text-sm text-slate-500 mb-4">
        How much work each person actually completed in a period you pick — Weekly, Monthly, Yearly, or any custom date range. "Sent Back / Rejected" is the same-period quality signal: how much of that output needed rework.
      </p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {PERF_PRESETS.map((p) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-medium border transition ${
                preset === p.id ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300'
              }`}
            >
              {p.label}
            </button>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        <Card className="p-3.5">
          <div className="flex items-center gap-2 text-slate-400 mb-1"><Users className="w-3.5 h-3.5" /><span className="text-xs font-medium">Team members</span></div>
          <p className="text-2xl font-semibold text-slate-900">{summary.people}</p>
        </Card>
        <Card className="p-3.5">
          <div className="flex items-center gap-2 text-slate-400 mb-1"><Check className="w-3.5 h-3.5" /><span className="text-xs font-medium">Completed</span></div>
          <p className="text-2xl font-semibold text-slate-900">{summary.totalCompleted}</p>
        </Card>
        <Card className="p-3.5">
          <div className="flex items-center gap-2 text-slate-400 mb-1"><XCircle className="w-3.5 h-3.5" /><span className="text-xs font-medium">Sent back / Rejected</span></div>
          <p className={`text-2xl font-semibold ${summary.totalSentBack > 0 ? 'text-amber-600' : 'text-slate-900'}`}>{summary.totalSentBack}</p>
        </Card>
        <Card className="p-3.5">
          <div className="flex items-center gap-2 text-slate-400 mb-1"><TrendingUp className="w-3.5 h-3.5" /><span className="text-xs font-medium">Top performer</span></div>
          <p className="text-sm font-semibold text-slate-900 truncate">
            {summary.topPerformer && summary.topPerformer.completed > 0 ? `${summary.topPerformer.full_name} (${summary.topPerformer.completed})` : '—'}
          </p>
        </Card>
      </div>
      ...
      <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-white border border-slate-300 hover:border-slate-400 text-slate-700 px-3 py-2 rounded-lg text-sm font-medium transition disabled:opacity-50"
          >
      ...
      {!isLoading && filteredRows.length === 0 && (
        <EmptyState icon={<BarChart3 className="w-10 h-10" />} title="No matching team members" subtitle="Try a different period, role, or search term." />
      )}
            ...
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                <th
                  ...
                  className="text-left px-4 py-3 font-medium text-slate-600 cursor-pointer select-none hover:text-slate-900 whitespace-nowrap"
                >
                  Person {sortKey === 'full_name' && <span className="ml-1 text-slate-400">{sortDir === 'asc' ? '↑' : '↓'}</span>}
                </th>
                <th className="text-left px-4 py-3 font-medium text-slate-600">Role</th>
                <th
                  ...
                  className="text-left px-4 py-3 font-medium text-slate-600 cursor-pointer select-none hover:text-slate-900 whitespace-nowrap"
                >
                  Completed {sortKey === 'completed' && <span className="ml-1 text-slate-400">{sortDir === 'asc' ? '↑' : '↓'}</span>}
                </th>
                <th
                  onClick={() => toggleSort('sentBack')}
                  className="text-left px-4 py-3 font-medium text-slate-600 cursor-pointer select-none hover:text-slate-900 whitespace-nowrap"
                >
                  Sent Back / Rejected {sortKey === 'sentBack' && <span className="ml-1 text-slate-400">{sortDir === 'asc' ? '↑' : '↓'}</span>}
                </th>
              </tr>
            </thead>
            <tbody>
              {filteredRows.map((r) => (
                <tr key={`${r.user_id}-${r.role}`} className="border-b border-slate-100 hover:bg-slate-50">
                  <td className="px-4 py-3 font-medium text-slate-900">
                    <span className="flex items-center gap-2.5">
                      <span className="w-7 h-7 rounded-full bg-indigo-100 text-indigo-700 text-[11px] font-semibold flex items-center justify-center shrink-0">
                        {initials(r.full_name)}
                  ...
                  <td className="px-4 py-3 font-semibold text-slate-900">{r.completed}</td>
                  <td className={`px-4 py-3 font-medium ${r.sentBack != null && r.sentBack > 0 ? 'text-amber-600' : 'text-slate-400'}`}>
                    {r.sentBack ?? '—'}
                  </td>
                </tr>
              ))}
