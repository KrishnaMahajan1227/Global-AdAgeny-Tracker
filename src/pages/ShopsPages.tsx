  ...
  useEffect(() => {
    ...
    if (workItems && workItems.length > 0) {
      setBackfillItems(workItems.map((w) => ({
        ...
        ...(() => {
          // Show the size exactly as it was entered (inch / ft / m / cm) — never the converted feet value.
          const fw = w.approved_width ?? w.survey_width, fh = w.approved_height ?? w.survey_height;
          const ok = (w as any).entered_width != null && (w as any).entered_height != null && (w as any).entered_width_unit && (w as any).entered_height_unit
            && Math.abs(toFeet(Number((w as any).entered_width), (w as any).entered_width_unit) - Number(fw)) < 0.05 && Math.abs(toFeet(Number((w as any).entered_height), (w as any).entered_height_unit) - Number(fh)) < 0.05;
          return ok
            ? { width: String((w as any).entered_width), height: String((w as any).entered_height), unit: (w as any).entered_width_unit as string, heightUnit: (w as any).entered_height_unit as string }
            : { width: fw == null ? '' : trimNum(fw), height: fh == null ? '' : trimNum(fh), unit: 'ft', heightUnit: 'ft' };
        })(),
      ...
      <Modal open={surveyPhotoUploadOpen} onClose={() => setSurveyPhotoUploadOpen(false)} title="Upload Survey Photos · Link to Board" size="lg">
        <div className="space-y-4">
          <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900">
            {surveyPhotoTargetItemId ? <>These photos will be attached directly to <b>{(() => { const x=(workItems||[]).find(w=>w.id===surveyPhotoTargetItemId); return x ? `${workItemName(x, (workItems||[]).indexOf(x))} · ${itemSizeLabel(x, x.approved_width != null ? 'approved' : 'survey')}` : 'this work item'; })()}</b>. No re-selection is required.</> : <>Bulk mode: map each uploaded survey photo to the exact work item / measurement it documents.</>}
          </div>
              ...
              {surveyPhotoUploadFiles.map((file, photoIndex) => <div key={`${file.name}-${photoIndex}`} className="p-3 grid grid-cols-[56px_1fr] gap-3 items-center">
                ...
                <div className="min-w-0"><div className="flex items-center gap-2 mb-1"><span className="text-xs font-bold text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded">Photo {photoIndex + 1}</span><span className="text-xs text-slate-500 truncate">{file.name}</span></div>
                  {surveyPhotoTargetItemId ? <div className="w-full border border-blue-200 bg-blue-50 rounded-lg px-2.5 py-2 text-sm text-blue-800 font-medium">Automatically linked to this Work Item</div> : <select value={surveyPhotoFileMap[photoIndex] || ''} onChange={(e) => setSurveyPhotoFileMap((prev) => ({ ...prev, [photoIndex]: e.target.value }))} className="w-full border border-slate-300 rounded-lg px-2.5 py-2 text-sm bg-white">
                    ...
                    {(workItems || []).map((item, index) => <option key={item.id} value={item.id}>Board {index + 1} · {workItemName(item, index)} · {itemSizeLabel(item, item.approved_width != null ? 'approved' : 'survey')} · Qty {item.approved_quantity ?? item.survey_quantity ?? 1}</option>)}
                  </select>}
                </div>
              </div>)}
            </div>
