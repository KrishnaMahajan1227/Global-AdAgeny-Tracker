...
function SurveyorHome({ onStart }: { onStart: (shopId: string) => void }) {
  ...
  // Without this, a shop Admin/Owner sends back with "Reject" or "Request
  // Correction" (Survey Review) never actually reappears here until the
  // surveyor force-closes and reopens the app — the shop_assignments/shops
  // rows update on the server the instant that happens, but nothing was
  // ever telling this screen to refetch. Same live-refresh pattern every
  // other role's Home screen already uses (Installer, office queues).
  useRealtimeInvalidate(['shop_assignments', 'shops', 'surveys'], orgId, [['surveyor-assignments', profile?.id]]);
  ...
  // The reviewer's own note on WHY this specific shop came back — shown
  // right on the Next Job card so a surveyor sees exactly what to fix
  // before they even start driving there, not just "assigned again" with
  // no explanation.
  const surveyableShopIds = activeAssignments.filter((a) => isShopSurveyable(a.shops?.status)).map((a) => a.shop_id);
  const { data: sentBackReviews } = useQuery({
    queryKey: ['surveyor-sent-back', profile?.id, surveyableShopIds.join(',')],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('surveys')
        .select('shop_id, status, review_note, reviewed_at')
        .in('shop_id', surveyableShopIds)
        .in('status', ['rejected', 'correction_requested'])
        .order('reviewed_at', { ascending: false, nullsFirst: false });
      if (error) { console.error('[SurveyorHome] could not load review notes:', error.message); return []; }
      // Latest per shop only — a shop can have more than one past
      // rejection/correction round; only the most recent one is relevant.
    ...
    enabled: surveyableShopIds.length > 0,
  });
  const nextJobReview = nextJob ? (sentBackReviews || []).find((r) => r.shop_id === nextJob.shop_id) : undefined;
    ...
    <div className="p-4">
      ...
      {nextJob && (
        <Card className="mt-4 p-4">
          ...
          {nextJobReview && (
            <div className={`mt-3 rounded-lg p-3 text-xs border ${nextJobReview.status === 'rejected' ? 'bg-red-50 border-red-200 text-red-700' : 'bg-amber-50 border-amber-200 text-amber-700'}`}>
              <p className="font-semibold flex items-center gap-1">
                <AlertCircle className="w-3.5 h-3.5" /> {nextJobReview.status === 'rejected' ? 'Sent back — Rejected' : 'Sent back — Correction Needed'}
              </p>
              {nextJobReview.review_note ? (
                <p className="mt-1">{nextJobReview.review_note}</p>
              ) : (
                <p className="mt-1 italic opacity-80">No note was left — re-check the boards and measurements before resubmitting.</p>
              )}
            </div>
          )}
        </Card>
...
function SurveyorWork({ onStart }: { onStart: (shopId: string) => void }) {
  ...
  const { data: sentBackReviews } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .in('status', ['rejected', 'correction_requested'])
        .order('reviewed_at', { ascending: false, nullsFirst: false });
      if (error) { console.error('[SurveyorWork] could not load review notes:', error.message); return []; }
      const seen = new Set<string>();
      const latest: typeof data = [];
      for (const row of data || []) {
        if (seen.has(row.shop_id)) continue;
  ...
  const reviewByShopId = new Map((sentBackReviews || []).map((r) => [r.shop_id, r]));
    ...
    <div className="p-4">
      ...
      <AssignedShopList
        ...
        renderExtra={(a) => {
          ...
          const review = reviewByShopId.get(a.shop_id);
          if (!review) return null;
          const isRejected = review.status === 'rejected';
          return (
            <div className={`mt-1.5 rounded-lg px-2.5 py-2 text-[11px] border ${isRejected ? 'bg-red-50 border-red-200 text-red-700' : 'bg-amber-50 border-amber-200 text-amber-700'}`}>
              <p className="font-semibold flex items-center gap-1">
                <AlertCircle className="w-3 h-3" /> {isRejected ? 'Rejected — resurvey needed' : 'Correction requested'}
              </p>
              {review.review_note && <p className="mt-0.5">{review.review_note}</p>}
            </div>
          );
        }}
      />
...
function SurveyWizard({ shopId, onExit }: { shopId: string; onExit: (nextShopId?: string) => void }) {
  ...
  // If this shop is being re-surveyed after Admin/Owner sent it back
  ...
  const { data: lastReview } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .limit(1)
        .maybeSingle();
      if (error) { console.error('[SurveyWizard] could not load last review:', error.message); return null; }
      return data;
    },
    enabled: !!shopId,
  });
          ...
          {lastReview && (
              <Card className={`p-4 border ${lastReview.status === 'rejected' ? 'border-red-300 bg-red-50' : 'border-amber-300 bg-amber-50'}`}>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className={`font-medium text-sm ${lastReview.status === 'rejected' ? 'text-red-800' : 'text-amber-800'}`}>
                    {lastReview.status === 'rejected' ? 'This survey was rejected' : 'Correction requested by Admin/Owner'}
                  </p>
                </div>
                <p className={`text-xs ${lastReview.status === 'rejected' ? 'text-red-700' : 'text-amber-700'}`}>
                  {lastReview.review_note || 'No note was left — re-check the boards and measurements carefully before resubmitting.'}
                </p>
              </Card>
            )}
