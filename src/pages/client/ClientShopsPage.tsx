...
export default function ClientShopsPage() {
  ...
  const hasActiveFilters = !!(search || zoneFilter || statusFilter || workOrderFilter || campaignFilter || cityFilter || districtFilter || stateFilter);
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-5">
        ...
        <p className="text-xs text-slate-400 shrink-0">{filteredShops.length} of {(shops || []).length} sites{hasActiveFilters ? ' match' : ' total'}</p>
      </div>
        ...
        {shopsToRender.map((shop) => {
          ...
          return (
            <Card key={shop.id} className="overflow-hidden hover:border-blue-300 transition">
              <button onClick={() => setDrawerShopId(shop.id)} className="w-full flex items-center justify-between gap-3 px-4 py-3 text-left">
